...
request_access_token_1(struct request_access_token_input *argp, CLIENT *clnt)
{
	static struct request_access_token_output clnt_res;

	memset((char *)&clnt_res, 0, sizeof(clnt_res));
	if (clnt_call (clnt, request_access_token,
		(xdrproc_t) xdr_request_access_token_input, (caddr_t) argp,
		(xdrproc_t) xdr_request_access_token_output, (caddr_t) &clnt_res,
		TIMEOUT) != RPC_SUCCESS) {
		return (NULL);
	}
	return (&clnt_res);
